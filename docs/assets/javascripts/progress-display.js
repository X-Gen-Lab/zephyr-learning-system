...
(function() {
  ...
  function extractPageIdFromHref(href) {
    ...
    // 或者：zephyr-learning-system -> null (仓库根目录就是首页)
    
    // 检查是否只是仓库名（没有后续路径）
    if (path === 'zephyr-learning-system') {
      return null; // 首页不标记
    }
    
    // 移除仓库名前缀
    if (path.startsWith('zephyr-learning-system/')) {
      path = path.replace(/^zephyr-learning-system\//, '');
